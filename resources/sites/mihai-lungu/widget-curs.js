...
async function fetchXMLRates(date, retryCount = 3) {
    ...
    // If all proxies failed, retry with delay
    if (retryCount > 0) {
        console.log(`All proxies failed. Retrying... Attempts left: ${retryCount}`);
    ...
    console.error('All proxies failed and no retries left');
    return null;
}
